...
from .retinasensor import FovealConeSensor

def make_eyes(res) -> Camera:
    prim_utils.create_prim("/World/Head/Eye_L", "Xform")
    prim_utils.create_prim("/World/Head/Eye_R", "Xform")
    camera_cfg = CameraCfg(
        ...
        #TODO-YatesLab make pinhole cam a fisheye cam with proper distortions
        spawn=sim_utils.PinholeCameraCfg(
            focal_length=24.0, focus_distance=400.0, horizontal_aperture=20.955, clipping_range=(0.1, 1.0e5)
        ),
    )
    # Create camera
    camera = FovealConeSensor(cfg=camera_cfg)
    return camera
...
class HumanOcularSystem:
    ...
    def set_world_poses_from_view(self, head_pos, look_at):
        self.eyes.set_world_poses_from_view(head_pos, look_at) # TODO-YatesLab add torsion
